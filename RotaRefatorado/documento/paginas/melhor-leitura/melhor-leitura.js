...
(() => {
  ...
  // ── Estado on/off (controlado pelo popup) ─────────────────
  const ML_ATIVO_KEY = 'melhorLeitura_ativo'
  let _mlAtivo = false  // default desligado

  NAV.storage.local.get(ML_ATIVO_KEY).then(result => {
    if (result[ML_ATIVO_KEY] === true) _mlAtivo = true
  }).catch(() => {})

  NAV.storage.onChanged.addListener((changes, area) => {
    if (area === 'local' && changes[ML_ATIVO_KEY] !== undefined) {
      _mlAtivo = changes[ML_ATIVO_KEY].newValue !== false
    }
  })

  window.addEventListener('rotapje:melhorleitura-atualizado', e => {
    ...
    if (!texto) {
      ...
      while (pai && nivel < 5) {
        texto = (
          ...
          pai.getAttribute('data-placeholder') ||
  ...
  document.addEventListener('click', e => {
    if (!e.altKey) return;
    if (!_mlAtivo) return;  // Melhor Leitura desativado pelo popup
    e.preventDefault();
    e.stopPropagation();
