...
async function balcao_virtualCriaCampoConsulta() {

    
    let WIDGET_ID   = 'pjerota-balcao-widget'
    let STORAGE_POS = 'balcao_widget_pos'
    ...
    titulo.textContent = 'Balcão Virtual'
    _s(titulo, {
        flex:          '1',
        fontWeight:    '700',
        fontSize:      '11px',
    ...
    btnLupa.textContent = '🔍'
    btnLupa.id          = 'pjerota-balcao-lupa'
    _s(btnLupa, {
        background:   C.azul,
        border:       'none',
        borderRadius: '7px',
    ...
    btnLupa.addEventListener('click', () => balcao_virtualConsultar(input.value.trim()))
...
async function balcao_virtualVerificasSeEstaAtiva() {
    
    let botaoRota = await aguardarElemento('#pjerota-btn-rota', 30000)
    if (!botaoRota) return
    let tarefaAtiva = await obterArmazenamento('tarefaAtiva')
    console.log(JSON.stringify(tarefaAtiva))
    if (tarefaAtiva.tarefaAtiva !== "balcao-virtual") return
    console.log('vou chamar o cria campo.')
    await balcao_virtualCriaCampoConsulta()
}

balcao_virtualVerificasSeEstaAtiva() 

NAVEGADOR.storage.onChanged.addListener((changes) => {
    if (!changes.tarefaAtiva) return
    if (changes.tarefaAtiva.newValue === 'balcao-virtual') {
        balcao_virtualCriaCampoConsulta()
    } else {
        document.getElementById('pjerota-balcao-widget')?.remove()
    }
})

async function balcao_virtualConsultar(numeroDoProcesso) {
    console.log(JSON.stringify(numeroDoProcesso))
    let numeroVerificado = ROTA_REGEX_CNJ.test(numeroDoProcesso)
    if (!numeroVerificado) {
        let campo =  document.getElementById('pjerota-balcao-input')
        campo.placeholder = 'Número não está no padrão CNJ.'
        campo.value = ''
        let lupa = document.getElementById('pjerota-balcao-lupa')
...
function balcao_virtualNavegar(url) {
    location.href = url
}