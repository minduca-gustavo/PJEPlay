...
async function triagem_inicial_janelaDetalhes(sessao){
    ...
    await rota_aguardarElemento('.tl-documento')
    ...
    for (let i = 0; i < 100; i++) {
        let cabecalho = await rota_aguardarElemento('.mat-card-title')
    ...
    await rota_aguardarElemento('[aria-label*="Anexos"]')
...
async function triagem_inicial_colocarGigDeAcompanhamento() {
    ...
    if (mostrarOuEsconder.ariaLabel == 'Mostrar o GIGS') {
        await suspender(1000)
        await clicar(mostrarOuEsconder)
    }
    ...
    for (let c of p?.comandos){
        ...
        const concluiu = await Promise.race([
            new Promise(resolver => {
                NAVEGADOR.storage.onChanged.addListener(function ouvir(mudancas) {
                    if (mudancas['rota_acoes_conjuntas_triagem_inicial_pronta']?.newValue === c) {
                        NAVEGADOR.storage.onChanged.removeListener(ouvir)
                        resolver(true)
                    }
                })
