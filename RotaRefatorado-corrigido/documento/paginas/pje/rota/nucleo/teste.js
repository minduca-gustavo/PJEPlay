...
(async ()=> {
    let testando = 0
    if (testando !== 1) return
    let d = await rota_extrairTeorDocumento(3866132, 279194446)
...
(function () {
  ...
  function rota_aguardarElemento(seletor, timeout = 10000) {
  ...
  async function executarDevolucao(btnEditar, nome, descricao) {
    ...
    try {
      ...
      await rota_aguardarElemento('[aria-label="Salva as alterações"]');
