...
async function _acao_fetch(url = '', metodo = 'GET', corpo = null) {
    const token    = cookie_obter('Xsrf-Token') || cookie_obter('XSRF-TOKEN')
...
async function acao_ui_abrirMenu(item = '') {
    const menu = await rota_aguardarElemento('[aria-label="Menu do processo"],[aria-label="Menu da tarefa"]', 5000)
    if (!menu) return relatar('acao_ui_abrirMenu: menu não encontrado', '', 'erro')
    await suspender(100)
    menu.click()

    if (item) {
        const botao = await rota_aguardarElemento(`[descricao="${item}"] button`, 3000)
...
async function acao_ui_selecionarOpcao(seletor = '', texto = '') {
    if (!texto) return
    const controle = await rota_aguardarElemento(seletor, 3000)
    if (!controle) return
    controle.click()
    await rota_aguardarElemento('mat-option', 3000)
    await suspender(100)
...
async function acao_ui_aguardarMensagem(texto = '', timeout = 8000) {
    ...
    const result  = await rota_aguardarElemento(xpath, timeout)
    return !!result
}
