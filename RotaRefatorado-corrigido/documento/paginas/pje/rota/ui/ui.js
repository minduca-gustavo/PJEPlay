...
/**
 * Paleta lida de estilos/rotapje-cores.css.
 *
 * As cores saíram do JS: a única fonte de verdade agora é a folha
 * de estilo, no mesmo modelo do SISE (tripla HSL em custom
 * property). Trocar a identidade visual do Rota é editar um
 * arquivo CSS, não caçar hex espalhado por 2 mil linhas.
 *
 * A leitura é feita uma vez, sob demanda, e memorizada — os
 * componentes continuam aplicando estilo inline como antes, então
 * nada mais neste arquivo precisou mudar.
 */
let _ui_paletaCache = null

function rota_ui_lerPaleta(){

	if(_ui_paletaCache) return _ui_paletaCache

	let raiz = getComputedStyle(document.documentElement)

	function cor(nome, reserva){
		let tripla = raiz.getPropertyValue('--extensao-rotapje-cor-' + nome).trim()
		if(!tripla) return reserva
		return 'hsl(' + tripla + ')'
	}

	_ui_paletaCache = {
		azul:         cor('azul',          '#0078aa'),
		azulHover:    cor('azul-hover',    '#005f8a'),
		azulClaro:    cor('azul-claro',    '#2a5a8c'),
		laranja:      cor('laranja',       '#ffa726'),
		laranjaHover: cor('laranja-hover', '#e6941f'),
		fundo:        cor('fundo',         '#f9f9fa'),
	...
	return _ui_paletaCache

}

/**
 * Proxy de leitura: UI_CORES.azul continua funcionando em todos os
 * pontos de uso existentes, mas resolve para o valor do CSS.
 */
const UI_CORES = new Proxy({}, {
	get: (_, chave) => rota_ui_lerPaleta()[chave],
	has: (_, chave) => chave in rota_ui_lerPaleta(),
	ownKeys: () => Reflect.ownKeys(rota_ui_lerPaleta()),
	getOwnPropertyDescriptor: () => ({ enumerable:true, configurable:true }),
})

function rota_ui_fonte(){
	return getComputedStyle(document.documentElement)
		.getPropertyValue('--extensao-rotapje-fonte').trim()
...
async function criaDivFlutuante({ id, titulo = '', largura = '280px', ancestral, armazenarRecolhido = false }) {
    ...
    if (armazenarRecolhido){
        let label = id + 'estado'
        let armazenamento = await obterArmazenamento([label])
        if (armazenamento) recolhido = armazenamento[label]
...
function criaTextoQueAbrePassandoOMouse({ id, texto = '📋 Passe o mouse', ancestral, corDoBox = 'azul', textoBox = '' }) {
    ...
    const box = _ui_el('div', {
        ...
        color: UI_CORES.texto, fontFamily: "var(--extensao-rotapje-fonte)",
