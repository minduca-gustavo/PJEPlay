...
async function criaWidgetAssistenteAssinatura(ancestral) {
    ...
    for(t of tipos){
        let checkBox = criaCheckBox({
            id: 'rota_assistenteAssinatura_check_' + t?.tipo, 
            textoAoLado: t?.label, 
            ancestral: 'rota_assistenteAssinatura-corpo',
        })
        checkBox.style.marginLeft = '3px'
        let checkListener = document.querySelector('#rota_assistenteAssinatura_check_' + t?.tipo)
        checkListener.dataset.tipo = t?.tipo
        checkListener.addEventListener('click', () => alternarCheckAssistenteAssinatura(checkListener))
    }
    async function alternarCheckAssistenteAssinatura(el) {
        let todosChecks = [...document.querySelectorAll('[id*="rota_assistenteAssinatura_check_"]')]
        
        for (t of tipos){
            let check = document.querySelector('#rota_assistenteAssinatura_check_' + t?.tipo)
    ...
    const CHAVE_CONFIGURACOES = 'rota_assistenteAssinatura_configuracoesUsuario'
    const CHAVE_ULTIMA_CONFIGURACAO = 'rota_assistenteAssinatura_ultimaConfiguracaoEscolhida'
    ...
    function _aplicarConfiguracao(nome) {
        ...
        if (!cfg) return
        let checkDesejado = document.querySelector('#rota_assistenteAssinatura_check_' + cfg?.tipo)
        if (checkDesejado && checkDesejado.dataset.marcado !== '1'){
            checkDesejado.click()
        }
        for (let j of cores){
            let inputEl = document.querySelector('#rota_assistenteAssinatura_divCores_input_' + j?.nome.toLowerCase())
    ...
    async function _renderizarMenuConfiguracoes(valorParaSelecionar) {
        let divMenu = document.getElementById('rota_assistenteAssinatura_divMenuConfig')
        if (!divMenu) return
        divMenu.innerHTML = ''
        ...
        if (!nomes.length){
            let vazio = criaTexto({
                id: 'rota_assistenteAssinatura_configVazia',
                texto: 'Nenhuma configuração salva ainda.',
                ancestral: 'rota_assistenteAssinatura_divMenuConfig',
            })
        ...
        menuConfiguracoes = criaMenuSuspenso({
            id: 'rota_assistenteAssinatura_menuConfiguracoes',
            opcoes: opcoes,
    ...
    async function _removerConfiguracaoAtual() {
        ...
        if (!nomeAtual || !configuracoesCache?.[nomeAtual]){
            rota_assistenteAssinatura_avisoTemporario('Nenhuma configuração selecionada para remover.')
            return
        }
        ...
        try {
            ...
            rota_assistenteAssinatura_avisoTemporario('Não foi possível remover a configuração. Tente novamente.')
            return
        }
        ...
        rota_assistenteAssinatura_avisoTemporario('Configuração "' + nomeAtual + '" removida.')
    }

    let subTituloConfigs = criaSubTitulo({
        id: 'rota_assistenteAssinatura_subTituloConfigs',
        texto: 'Configurações salvas',
        ancestral: 'rota_assistenteAssinatura-corpo',
    })
    let divLinhaConfiguracoes = criaDiv({
        id: 'rota_assistenteAssinatura_divLinhaConfiguracoes',
        ancestral: 'rota_assistenteAssinatura-corpo',
        rowColumn: 'row',
    })
    let divMenuConfig = criaDiv({
        id: 'rota_assistenteAssinatura_divMenuConfig',
        ancestral: 'rota_assistenteAssinatura_divLinhaConfiguracoes',
    })
    divMenuConfig.style.flex = '1'
    divMenuConfig.style.marginBottom = '0'
    let divBotaoRemoverConfig = criaDiv({
        id: 'rota_assistenteAssinatura_divBotaoRemoverConfig',
        ancestral: 'rota_assistenteAssinatura_divLinhaConfiguracoes',
    })
    divBotaoRemoverConfig.style.marginBottom = '0'
    criaBotaoLaranja({
        id: 'rota_assistenteAssinatura_botaoRemoverConfig',
        texto: '🗑',
        ancestral: 'rota_assistenteAssinatura_divBotaoRemoverConfig',
    ...
    for (let j of cores){
        ...
        let idDivCores = 'rota_assistenteAssinatura_divCores' + j?.nome.toLowerCase()
        ...
        for (let k of secoes){
            ...
            let divSecao = criaDiv({
                id: 'rota_assistenteAssinatura_divCores' + j?.nome.toLowerCase() + '_' + k?.nome,
            ...
            if (funcao){
                let inputNome = 'rota_assistenteAssinatura_divCores_input_' + j?.nome.toLowerCase()
    ...
    */
    ...
    let divSalvarConfig = criaDiv({
        id: 'rota_assistenteAssinatura_divSalvarConfig',
        ancestral: 'rota_assistenteAssinatura-corpo',
        rowColumn: 'row',
    })
    let divInputNomeConfig = criaDiv({
        id: 'rota_assistenteAssinatura_divInputNomeConfig',
        ancestral: 'rota_assistenteAssinatura_divSalvarConfig',
    })
    divInputNomeConfig.style.flex = '1'
    divInputNomeConfig.style.marginBottom = '0'
    let inputNomeConfig = criaInput({
        id: 'rota_assistenteAssinatura_inputNomeConfig',
        ancestral: 'rota_assistenteAssinatura_divInputNomeConfig',
        placeholder: 'Nome da configuração',
    })
    let divBotaoSalvarConfig = criaDiv({
        id: 'rota_assistenteAssinatura_divBotaoSalvarConfig',
        ancestral: 'rota_assistenteAssinatura_divSalvarConfig',
    })
    divBotaoSalvarConfig.style.width = '40px'
    divBotaoSalvarConfig.style.marginBottom = '0'
    criaBotaoAzul({
        id: 'rota_assistenteAssinatura_botaoSalvarConfig',
        texto: '+',
        ancestral: 'rota_assistenteAssinatura_divBotaoSalvarConfig',
        acao: () => _salvarConfiguracaoAtual()
    })

    async function _salvarConfiguracaoAtual() {
        let nomeInput = document.querySelector('#rota_assistenteAssinatura_inputNomeConfig')
        let nome = nomeInput?.value?.trim() || ''
        if (nome === ''){
            rota_assistenteAssinatura_avisoTemporario('Dê um nome para a configuração antes de salvar.')
            return
        }

        let checkMarcado = tipos.find(t => document.querySelector('#rota_assistenteAssinatura_check_' + t?.tipo)?.dataset.marcado === '1')
        if (!checkMarcado){
            rota_assistenteAssinatura_avisoTemporario('Selecione um tipo de documento (Petições, Ata ou Sentenças) antes de salvar.')
            return
        }
        ...
        if (!algumTermoPreenchido){
            rota_assistenteAssinatura_avisoTemporario('Preencha ao menos um campo de termos antes de salvar.')
            return
        }
        ...
        try {
            ...
            rota_assistenteAssinatura_avisoTemporario('Não foi possível salvar a configuração. Tente novamente.')
            return
        }
        ...
        rota_assistenteAssinatura_avisoTemporario(jaExistia ? 'Configuração "' + nome + '" atualizada.' : 'Configuração "' + nome + '" salva.')
    }
